...
export default function CheckWriter() {
  ...
  return <div className="p-4 md:p-6 space-y-5">
    <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4"><div><div className="flex items-center gap-2 text-primary text-xs font-semibold uppercase tracking-widest"><Printer className="w-4 h-4" /> Independent disbursement</div><h1 className="text-3xl font-bold tracking-tight mt-1">Check Writer</h1><p className="text-sm text-muted-foreground mt-1">Write, record, and print Philippine bank checks independently.</p></div><div className="flex gap-3"><div className="bg-card border border-border rounded-xl px-4 py-3"><p className="text-[10px] uppercase tracking-widest text-muted-foreground">Checks</p><p className="text-xl font-bold">{writer.checks.length}</p></div><div className="bg-card border border-border rounded-xl px-4 py-3"><p className="text-[10px] uppercase tracking-widest text-muted-foreground flex items-center gap-1"><Landmark className="w-3 h-3" /> Recorded value</p><p className="text-xl font-bold font-mono">₱{total.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p></div></div></header>
