...
export default function CompactClientPaymentGroup({ clientName, listings, clients, onAddPayment, onAssignBuyer }) {
  ...
  const finalPrice = listings.reduce((sum, item) => sum + (item.price_breakdown?.base || 0), 0);
  ...
  return (
    <div className="bg-card">
      <button className="w-full px-5 py-3 hover:bg-muted/50 transition-colors" onClick={() => setExpanded(!expanded)}>
        <div className="grid grid-cols-[1.5fr_1.2fr_1fr_1fr_1fr_1fr_1fr_1fr_7rem_2.5rem] gap-3 items-center text-sm">
          ...
          <span className="text-right">{fmt(finalPrice)}</span>
