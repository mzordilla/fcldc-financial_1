...
export default function CondoSaleBreakdown({ totalPrice }) {
  const basePrice = Number(totalPrice || 0) / 1.19;
  const closingFees = basePrice * 0.07;
  ...
  return (
    <div className="rounded-xl border border-border bg-muted/20 p-3">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div><p className="text-xs text-muted-foreground">Total Sale Price</p><p className="font-bold text-primary">{fmt(totalPrice)}</p></div>
      </div>
    </div>
  );
}