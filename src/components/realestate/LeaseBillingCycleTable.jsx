...
export default function LeaseBillingCycleTable({ cycles, loading, processing, onEdit, onDelete, onApprove, onReject }) {
  ...
  return <div className="overflow-x-auto rounded-2xl border border-border bg-card"><table className="w-full text-sm">
    ...
    <tbody className="divide-y divide-border">{cycles.map((cycle) => <tr key={cycle.id} className="hover:bg-muted/20">
      ...
      <td className="px-3 py-2"><p className="font-medium">{cycle.tenant_name}</p><p className="text-xs text-muted-foreground">{cycle.unit_number}</p>{cycle.tenant_contracts?.some((item) => item.contract_url) && <div className="mt-1 flex flex-wrap gap-2">{cycle.tenant_contracts.filter((item) => item.contract_url).map((item) => <a key={item.tenant_id} href={item.contract_url} target="_blank" rel="noreferrer" className="text-xs text-primary hover:underline">{item.unit_number || "Contract"}</a>)}</div>}</td>
