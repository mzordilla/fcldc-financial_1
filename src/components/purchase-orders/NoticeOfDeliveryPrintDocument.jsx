...
export default function NoticeOfDeliveryPrintDocument({ po }) {
  const items = po.line_items || [];

  return (
    <div className="min-h-[297mm] bg-white p-[15mm] text-black">
      ...
      <p className="mt-6 text-sm text-gray-600">This document confirms that the goods or services below have been delivered.</p>
      <div className="mt-6 grid grid-cols-2 gap-5 rounded-md border border-gray-300 bg-gray-50 p-4 text-sm">
      ...
      <div className="mt-6 text-sm"><p className="text-xs font-semibold uppercase text-gray-500">Description</p><p className="mt-1">{po.description || "—"}</p></div>
      ...
      {po.delivery_notes && <div className="mt-6 text-sm"><p className="text-xs font-semibold uppercase text-gray-500">Delivery Notes</p><p className="mt-1">{po.delivery_notes}</p></div>}
      <div className="mt-24 grid grid-cols-2 gap-16 text-center text-sm"><div className="border-t border-black pt-2">Received by / Signature</div><div className="border-t border-black pt-2">Authorized by / Signature</div></div>
    </div>
  );
}