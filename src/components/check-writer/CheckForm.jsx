...
export default function CheckForm({ bankAccounts, approvedRequests, loadingRequests, onSave, saving }) {
  ...
  const selectRequest = request => {
    ...
    if (!request) return setForm(current => ({ ...initial, bank_account_id: current.bank_account_id, check_number: current.check_number, check_date: current.check_date }));
    const amount = (request.amount || 0) - (request.withholding_tax_amount || 0) + (request.vat_amount || 0);
    setForm(current => ({ ...current, payee: request.payee, amount: String(amount), memo: request.description || "Payment request disbursement", source: "payment_approval", payment_request_ids: [request.id], payment_request_numbers: [request.request_number || request.invoice_number || request.id] }));
  };
  const submit = async print => {
    ...
    try { await onSave({ ...form, amount: Number(form.amount), amount_in_words: words }, print, printWindow); setForm(initial); setSelectedRequestId(""); }
  ...
  return <section className="bg-card border border-border rounded-2xl p-5 space-y-4">
    ...
    <ApprovedPRSelect requests={approvedRequests} selectedId={selectedRequestId} onSelect={selectRequest} loading={loadingRequests} />
    ...
    <div className="space-y-1.5"><Label>Pay to the Order of *</Label><Input value={form.payee} onChange={e => set("payee", e.target.value)} disabled={!!selectedRequestId} /></div>
    <div className="grid grid-cols-2 gap-3"><div className="space-y-1.5"><Label>Amount *</Label><Input type="number" min="0" step="0.01" value={form.amount} onChange={e => set("amount", e.target.value)} disabled={!!selectedRequestId} /></div><div className="space-y-1.5"><Label>Check Number *</Label><Input value={form.check_number} onChange={e => set("check_number", e.target.value)} /></div></div>
