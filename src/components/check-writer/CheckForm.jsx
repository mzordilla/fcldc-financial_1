...
export default function CheckForm({ bankAccounts, onSave, saving }) {
  ...
  const submit = async print => {
    ...
    try { await onSave({ ...form, amount: Number(form.amount), amount_in_words: words }, print, printWindow); setForm(initial); }
  ...
  return <section className="bg-card border border-border rounded-2xl p-5 space-y-4">
    ...
    <div className="space-y-1.5"><Label>Pay to the Order of *</Label><Input value={form.payee} onChange={e => set("payee", e.target.value)} /></div>
