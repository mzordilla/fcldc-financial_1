...
export default function CheckWriterWorkspace() {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid grid-cols-1 xl:grid-cols-[380px_minmax(0,1fr)] gap-5 items-start"><CheckForm bankAccounts={writer.bankAccounts} approvedRequests={availableRequests} payees={payees} loadingRequests={loadingRequests} onSave={writer.save} saving={writer.saving} /><CheckRegister checks={writer.checks} selected={writer.selected} onToggle={writer.toggle} onPrint={writer.printOne} onDelete={writer.deleteCheck} deletingId={writer.deletingId} onBatchPrint={writer.batchPrint} loading={writer.isLoading} /></div>
  </div>;
}