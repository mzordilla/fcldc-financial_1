...
const DEFAULT_ACCESS = {
  disbursement: ["/", "/receivables", "/bank-accounts", "/bank-reconciliation", "/transactions", "/payees", "/payment-approvals", "/check-writer", "/payables", "/chart-of-accounts", "/billing-cycles", "/receiving-items"],
...
function getAccessConfig() {
  try {
    ...
    if (config.disbursement && !config.disbursement.includes("/check-writer")) {
      config.disbursement = ["/check-writer", ...config.disbursement];
    }
    return config;
  } catch {
